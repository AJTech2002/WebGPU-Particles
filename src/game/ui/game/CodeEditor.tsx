...
`;
  ...
  const squad : SquadDef = squadId !== -1 ? (squadState.get(squadId) ?? nullSquad) : nullSquad;
  ...
  const save = () => {
    const transpiledCode = saveFile(
      squad.code.substring(preCode.length, squad.code.length)
    );
    ...
    updateSquad(squad.id, { transpiledCode: transpiledCode, code: squad.code });
          ...
          >
            ...
            {editingName === false ? (
              ...
              >
                ...
                {" "}
                {
                  squad.name === "" ? "Squad Name" : squad.name
                }{" "}
                </b>
              </p>
            ) : (
            ...
            <input
              type="color"
              value={
                squad.color === "" ? "#000000" : squad.color
              }
              onChange={(e) => {
            ...
            <ul>
              {squad.unitTypes.map((unitType) => {
                return (
                  <div key={unitType.type.toString()+"_DIV"} style={{
                    display: "flex",
                    flexDirection: "row",
                    justifyContent: "center",
                    alignItems: "center",
                  ...
                  <input key={+"_INP"} type="number" value={unitType.count} onChange={(e) => {
                    updateSquadUnitType(squad.id, unitType.type, Number.parseFloat(e.target.value));
                  }} />
                  </div>
                );
              })}
