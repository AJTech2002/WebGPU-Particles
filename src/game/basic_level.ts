...
import { UnitType } from "./boid_scene";
import { SquadManager } from "./components/managers/squad_manager";
...
export class BasicLevel extends BaseLevelScene {
  ...
  async init() {
    ...
    this.squadManager.spawnSquads(this.settings.squads);
    await this.seconds(1);
  ...
  async waitForGroup(group: Group, position: Vector3, minDelay: number = 0, maxDelay: number = 0) {
    ...
    const targetCastle = this.castles[Math.floor(Math.random() * this.castles.length)];
    for (let j = 0; j < group.numUnits; j++) {
      const enemy = this.spawnEnemy(position, "Soldier")
      const enemyComponent = enemy.gameObject.getComponent<BaseEnemy>(BaseEnemy);
      (enemyComponent as BaseEnemy).setTargetCastle(targetCastle);

      groupUnits.push(enemy);
    }
  ...
  start(): void {
    super.start();
    this.squadManager = this.gameManager.addComponent(new SquadManager());
    this.init();
    this.castles = this.findObjectsOfType<Castle>(Castle);
  }
